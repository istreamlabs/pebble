...
describe('FieldNumber', () => {
  it('renders without crashing', () => {
    expect(() => { shallow(<FieldNumber id="testField" label="test" />); }).not.toThrow();
  });

  it('allows for custom classes', () => {
    const wrapper = shallow(<FieldNumber id="testField" label="test" className="myClass" />);
    expect(wrapper.prop('className')).toContain('myClass');
  });

  describe('getLabelMarkup', () => {
    it('sets aria-label if hideLabel is true', () => {
      const wrapper = shallow(<FieldNumber id="testField" label="hiddenLabel" hideLabel />);
  ...
  describe('getHelpTextMarkup', () => {
    it('display the correct help text if passed', () => {
      ...
      const wrapper = shallow(<FieldNumber id="testField" label="label" helpText={helpText} />);
  ...
  describe('getValidationTextMarkup', () => {
    it('renders validationText if isInvalid', () => {
      ...
      const wrapper = shallow(<FieldNumber id="testField" label="label" validationText={validationText} isInvalid />);
      expect(wrapper.find(Text).childAt(0).text()).toBe(validationText);
    });
    it('returns undefined if isInvalid is false', () => {
      const instance = new FieldNumber({ label: 'label', validationText: 'this is invalid' });
    ...
    it('returns undefined if isInvalid is true but no validationText is set', () => {
      const instance = new FieldNumber({ label: 'label', isInvalid: true });
      expect(instance.getValidationTextMarkup()).toBe(undefined);
    });
  });
});
